...
class OP_estimator:
    ...
    def estimate_drawing(self, nextImg, images):
        preImg = self.preImg
        deltaT = self.deltaT
        h = self.h
        f = self.f
        ...
        draw_image = vu.drawFlow(self.preImg, good_old, good_new, pause = False, show = False)
        # font
        ...
        draw_image = cv2.putText(draw_image,f"estimated_Vl = {final_V_long}", org1, font, 
                   fontScale, color, thickness, cv2.LINE_AA)
        draw_image = cv2.putText(draw_image,f"estimated_w = {final_w}", org2, font, 
                   fontScale, color, thickness, cv2.LINE_AA)
